...
tfobj *listPopType(tfctx *ctx, int type){
    ...
    tfobj *to_pop = stack->list.ele[stack->list.len-1]; // Get the top element
    // Check to_pop type
    if(type!=TFOBJ_TYPE_ALL && to_pop->type!=type) return NULL;

    stack->list.len--;
    if(stack->list.len==0){         // if the stack is empty
...
/* Pop the object on the interpreter stack. */
tfobj *ctxStackPop(tfctx *ctx, int type){
    return listPopType(ctx, type);
}

/* Push the object on the interpreter stack. */
